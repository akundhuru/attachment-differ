...
\author{Ankitha Kundhuru\\[2pt]
  \small Independent Researcher\\
  \small \texttt{akundhuru@cs.stonybrook.edu}}
\date{\today}
...
document corpus (GovDocs1), extractors diverge on 46.5\% of attachment-pair
comparisons, and two independent content detectors (a deterministic heuristic
and Claude) both flip malicious$\rightarrow$benign on 31.6\% of extractor pairs
...
\begin{enumerate}
  ...
  per-format/vector breakdowns. On real documents it measures a 46.5\% pairwise
  ...
  modern content detector's verdict---including Claude.
...
\paragraph{Shadow Attacks~\cite{shadowattacks}.} Hide/replace content in
\emph{signed} PDFs so a viewer renders content the signature-validation logic
does not see---a \emph{renderer-vs-signature} gap in a document-integrity threat
model. Ours is orthogonal: an \emph{extractor-vs-extractor} (and
extractor-vs-render) gap in a content-detection threat model, with no signature
or single privileged view.

These works form an explicit lineage: Shadow Attacks itself extends PDF Mirage's
object-level \emph{extracted $\neq$ rendered} insight into the
signature-validation model; we extend the same insight into the \emph{extraction}
model, across multiple extractors.
...
attachment loader. From the Apache SpamAssassin public corpus we process all four
spam archives (\code{20021010}/\code{20030228}/\code{20030228\_2}/\code{20050311\_2};
2{,}400 messages, not deduplicated across the superseded 2002 release), recovering
31 attachments---of which only one is an Office document (25 are images, 4 HTML).
Document attachments are rare here, itself a finding about where document-parser
evasion lives. For a document-rich sample
...
pdfbox & \code{\textbackslash x01} (raw control byte) & 12{,}519 \\
\bottomrule
\end{tabular}
\caption{One glyph, four readings on identical bytes. For pdfbox, these raw
control bytes make up ${\sim}$40\% of its extracted text.}
\end{table}
...
reproducible) and Claude (Haiku 4.5), producing a structured-output verdict. An
...
Claude (Haiku 4.5) & 5 & 2 & \textbf{6/19 (31.6\%)} \\
\bottomrule
\end{tabular}
...
current release (olefile v0.47), and is addressed by a one-line complement to the
\#103 fix. We
...
including Claude (31.6\% of extractor pairs on image-masking vectors), and showed
